...
import DynamicFormModal from "@/components/forms/diary/DiaryFormModal";
import { GetDiaryBySchool } from "@/services/diaryServices";
...
const Diary = () => {
  ...
  const [diaryDataLen, setDiaryDataLen] = useState(0);
  ...
  const fetchDiaryEntries = async () => {
    try {
      ...
      const response = await GetDiaryBySchool(auth.roleId);
      let entries = response.data.diaries;
      setDiaryDataLen(entries?.length || 0);
      if (
        auth.role === "staff" &&
        ...
        entries = entries.filter((entry: any) => {
          return entry.staff_id === auth.userId;
        });
      }
      ...
      const structuredData = entries.map((entry: any) => ({
        ...
        ID: entry.diary_id,
        Status: entry.diary_status.toLowerCase(),
        "Staff name": entry.staff_name,
        Diary: entry.diarys,
        Date: entry.date,
        ModifiedBy: `Modified by ${entry.diary_modified_by}`,
      }));

      setData(structuredData);
    } catch (err: any) {
      setMessageStatus(err.message || "Failed to fetch diary entries");
      setStatus("error");
      setVisible(true);
    } finally {
      ...
      <View className="flex-row items-center justify-between px-4 py-3 border-b border-gray-200 bg-white">
        ...
        {searchVisible ? (
          ...
          <View className="h-10 flex-row items-center justify-center gap-2.5">
            <Text className="text-xl font-semibold">DIARY</Text>
            <Image
              source={images.diary}
