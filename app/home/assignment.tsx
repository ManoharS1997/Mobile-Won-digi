...
import AssignFormModal from "@/components/forms/assignment/AssignFormModal";
...
const Assignment = () => {
  ...
  const fetchAssignEntries = async () => {
    try {
      ...
      const response = await GetAssignment(auth.roleId);
      let entries = response.data.assignments;
      ...
      const structuredData = entries.map((entry: any) => ({
        Title: entry.assignment_name,
        ID: entry.assignment_id,
        Status: entry.assignment_status.toLowerCase(),
        "Staff name": entry.staff_name,
        Subject: entry.subject,
        Submission: entry.submission_date,
        Marks: entry.marks,
      }));
      ...
      setMessageStatus(
        err?.response?.data.message || "Failed to fetch assignment entries"
      );
      setStatus("error");
      setVisible(true);
    } finally {
      ...
      <View className="flex-row items-center justify-between px-4 py-3 border-b border-gray-200 bg-white">
        ...
        {searchVisible ? (
          ...
          <View className="h-10 flex-row items-center justify-center gap-2.5">
            <Text className="text-xl font-semibold">ASSIGNMENT</Text>
            <Image
              source={images.assignment}
