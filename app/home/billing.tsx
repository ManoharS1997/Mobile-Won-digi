...
import { GetBill } from "@/services/studentServices";
...
const Billing = () => {
  ...
  const fetchAssignEntries = async () => {
    try {
      ...
      const response = await GetBill(auth.roleId);
      let entries = response.data.bills;

      if (auth.role === "student") {
        entries = entries.filter((entry: any) => {
          return entry.user_id === auth.userId;
        });
      }
      ...
      const structuredData = entries.map((entry: any) => ({
        Title: `${entry.service_name} (Class ${entry.user_class}-${entry.user_section})`,
        ID: entry.bill_id,
        Status: entry.bill_status,
        "User name": entry.user_name,
        "Total fee": entry.total_fee,
        Submission: entry.due_date,
        "Fee paid": entry.fee_paid,
      }));

      setData(structuredData);
    } catch (err: any) {
      setMessageStatus(
        err.response.data.message || "Failed to fetch billing entries"
      );
      setStatus("error");
      setVisible(true);
    } finally {
