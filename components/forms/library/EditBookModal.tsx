...
import { UpdateBook } from "@/services/libraryServices";
...
const EditBookModal: React.FC<EditBookModalProps> = ({
  ...
  useEffect(() => {
    if (details) {
      setForm({
        ...
        book_publishers: details.book_publishers || "",
        book_purchase_date: details.book_purchase_date || "",
        book_category: details.book_category || "",
        book_price: details.book_price || "",
        aisle_number: details.aisle_number || "",
        rack_number: details.rack_number || "",
        quadrant: details.quadrant || "",
        book_status: details.book_status || "",
        book_school: details.book_school || auth.roleId || "",
        books: details.books || [],
      });
    }
  }, [details]);
  ...
  const handleDateConfirm = (date: Date) => {
    handleChange("book_purchase_date", format(date, "yyyy-MM-dd"));
  ...
  const handleSubmit = async () => {
    const requiredFields = bookFields;
    ...
    try {
      const response = await UpdateBook(auth.roleId || "", form.book_id, form);

      if (response?.data?.success) {
        setMessageStatus(response.data.message || "Book updated!");
        setStatus("success");
        setVisible(true);
        onSubmit(form);
      } else {
        ...
        <View className="flex-1 justify-end bg-black/30">
          <View className="bg-white rounded-t-3xl p-5 max-h-[90%]">
            <View className="flex-row justify-between items-center mb-4">
              <Text className="text-lg font-semibold">Edit Book</Text>
              <TouchableOpacity
              ...
              >
                ...
                >
                  ...
                  <Pressable
                    ...
                    className="bg-primary mt-5 p-3 rounded-md"
                  >
