...
import { GetCalendarById, UpdateCalendar } from "@/services/calendarServices";
...
const EditCalendarForm = ({
  ...
  const fetchCalendarById = async () => {
    try {
      if (!auth.roleId) {
        setMessageStatus("School ID or activity ID is missing");
        setStatus("error");
        setVisible(true);
        return;
      }

      const response = await GetCalendarById(auth.roleId, activityId);
      let entries = response.data?.event[0] || {};
      setFormData({
        ...formData,
        event_title: entries.event_title || "",
        event_description: entries.event_description || "",
        event_start_date: entries.event_start_date || "",
        event_end_date: entries.event_end_date || "",
        event_created_by: entries.event_created_by || auth.email,
        event_schoolid: entries.event_schoolid || auth.roleId,
      });
      setStartDate(entries.event_start_date?.split("T")[0] || null);
      setEndDate(entries.event_end_date?.split("T")[0] || null);
    } catch (err: any) {
  ...
  useEffect(() => {
    if (modalVisible) fetchCalendarById();
  }, [activityId, modalVisible]);
  ...
  const handleSaveActivity = async () => {
    ...
    try {
      const response = await UpdateCalendar(
        formData.event_schoolid || "",
        activityId,
        newEventData
      );
      if (response?.data?.success) {
        setMessageStatus("Activity edited successfully!");
        setStatus("success");
        setVisible(true);
      } else {
