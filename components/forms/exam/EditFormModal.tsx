...
const EditExamModal: React.FC<EditExamModalProps> = ({
  ...
  const [form, setForm] = useState<any>({
    timetable: "[]", // Initialize as an empty stringified array
  });
  ...
  useEffect(() => {
    if (details) {
      setForm((prev: any) => ({
        ...
        exam_name: details?.examName || "",
        start_date: details?.start_date || "",
        end_date: details?.end_date || "",
        exam_classes: details?.exam_classes || "",
        exam_status: details?.Status || "",
        timetable: JSON.stringify(details?.timetable || []),
      }));
      setTimetable(details?.timetable || []);
    }
  }, [details]);

  useEffect(() => {
    setForm((prev: any) => ({ ...prev, timetable: JSON.stringify(timetable) }));
  }, [timetable]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await CreateExam(form.exam_schoolid, details.ID, form);
      if (response?.data?.success) {
        setMessageStatus("Exam updated successfully!");
        setStatus("success");
        setVisible(true);
        onSubmit(form);
      } else {
        ...
        <View className="flex-1 justify-end bg-black/30">
          <View className="bg-white rounded-t-3xl p-5 max-h-[90%]">
            <View className="flex-row justify-between items-center mb-4">
              <Text className="text-lg font-semibold">Edit Exam</Text>
              <TouchableOpacity
