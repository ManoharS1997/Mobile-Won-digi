...
import { PostDiary } from "@/services/diaryServices";
...
const DiaryFormModal: React.FC<DiaryFormModalProps> = ({
  ...
  const generateDiaryId = (length: number) => {
    const idNumber = length + 1;
    return `D${String(idNumber).padStart(7, "0")}`;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await PostDiary(form);
      if (response?.data?.success) {
        setMessageStatus(
          response.data.message || "Diary entry created successfully!"
        );
        setStatus("success");
        setVisible(true);
        onSubmit(form);
      ...
      setMessageStatus(
        error?.response?.data?.message || "Failed to submit diary entry."
      );
      setStatus("error");
      setVisible(true);
    } finally {
        ...
        <View className="flex-1 justify-end bg-black/30">
          <View className="bg-white rounded-t-3xl p-5 max-h-[90%]">
            <View className="flex-row justify-between items-center mb-4">
              <Text className="text-lg font-semibold">Add Diary</Text>
              <TouchableOpacity
